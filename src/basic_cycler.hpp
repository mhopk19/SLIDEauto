...
struct CyclerData
{
	double Iout;	   // current of the cell at every step [A]
	double Vout;	   // voltage of the cell at every step [V]
	double OCVpout;	   // cathode potential of the cell at every step [V]
	double OCVnout;	   // anode potential of the cell at every step [V]
	double Tout;	   // temperature of the cell at every step [K]
	double timeChaout; // cumulative time spent on charging since the start at every step [s]
	double AhChaout;   // cumulative charged throughput since the start at every step [A]
	double WhChaout;   // cumulative charged energy throughput since the start at every step [Wh]
	double timeDisout; // cumulative time spent on discharging since the start at every step [s]
	double AhDisout;   // cumulative discharged charge throughput since the start at every step [A]
	double WhDisout;   // cumulative discharged energy throughput since the start at every step [Wh]
	double timeResout; // cumulative time spent on rest since the start at every step [s]
};
...
protected:
	...
	std::vector<double> Iout;		// current of the cell at every step [A]
	std::vector<double> Vout;		// voltage of the cell at every step [V]
	std::vector<double> OCVpout;	// cathode potential of the cell at every step [V]
	std::vector<double> OCVnout;	// anode potential of the cell at every step [V]
	std::vector<double> Tout;		// temperature of the cell at every step [K]
	...
	std::vector<double> timeDisout; // cumulative time spent on discharging since the start at every step [s]
	...
	std::vector<double> OCVni_vec, OCVpi_vec; // Created to use this vectors in getOCV for not creating every time.
	...
	void findCVcurrent_recursive(double Imin, double Imax, int sign, double Vset, double dt, bool blockDegradation, double *Il, double *Vl);
