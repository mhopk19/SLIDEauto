...
BasicCycler::BasicCycler(Cell &ci, std::string IDi, int verbosei, int CyclingDataTimeIntervali)
	: c(ci), ID(IDi), verbose(verbosei), CyclingDataTimeInterval(std::max(CyclingDataTimeIntervali, 0)), // CyclingDataTimeIntervali cannot be negative.
	  index(0), fileIndex(0), maxLength(100000),
	  timeCha(0), timeDis(0), timeRes(0), AhCha(0), AhDis(0), WhCha(0), WhDis(0)
{
	/*
	...
	Iout.reserve(maxLength);
	Vout.reserve(maxLength);
	OCVpout.reserve(maxLength);
	OCVnout.reserve(maxLength);
	Tout.reserve(maxLength);
	timeChaout.reserve(maxLength);
	AhChaout.reserve(maxLength);
	WhChaout.reserve(maxLength);
	timeDisout.reserve(maxLength);
...
void BasicCycler::writeCyclingData(const std::string &name, bool clear)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "BasicCycler::writeCyclingData(string, bool) starting\n";

	if (CyclingDataTimeInterval != 0 && !Tout.empty())
	{
		...
		{ // the file could not be opened for some reason. E.g. because the subfolder doesn't exist
			std::cerr << "ERROR on BasicCycler::writeCyclingStates(string, bool). File " << fullName
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerDetail)
			std::cout << "BasicCycler::writeCyclingData(string, bool) didn't write data because we are not collecting data (time resolution is "
					  << CyclingDataTimeInterval << ") or because there is no new data (number of data points so far:  " << Tout.size() << '\n';
	}

	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "BasicCycler::writeCyclingData(string, bool) terminating\n";
}
...
void BasicCycler::returnCyclingData(std::vector<double> &Ah, std::vector<double> &V, std::vector<double> &T)
{
	...
	for (size_t i = 0; i < AhChaout.size(); i++)
		Ah.push_back(std::abs(AhChaout[i]) + std::abs(AhDisout[i]));
...
void BasicCycler::storeResults(double I, double v, double ocvp, double ocvn, double tem)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerDetail) // note: this function is called at a high time-granularity so only print its start and end if we want to print details
	...
	if (CyclingDataTimeInterval != 0 && newdata)
	{
		...
		Iout.push_back(I);
		Vout.push_back(v);
		OCVpout.push_back(ocvp);
		OCVnout.push_back(ocvn);
		Tout.push_back(tem);
		timeChaout.push_back(timeCha);
		AhChaout.push_back(AhCha);
		WhChaout.push_back(WhCha);
		timeDisout.push_back(timeDis);
		AhDisout.push_back(AhDis);
		WhDisout.push_back(WhDis);
		timeResout.push_back(timeRes);
...
int BasicCycler::setCurrent(double I, double Vupp, double Vlow)
{
	...
	slide::State sini;	   // initial state of the cell
	...
	try
	{
		c.setI(settings::verbose >= printLevel::printCrit, check, 0); // set the current to 0
		c.getVoltage(settings::verbose >= printLevel::printCrit, &v, &ocvp, &ocvn, &etap, &etan, &rdrop, &tem);
	}
	catch (int e)
	{
	...
	try
	{
		...
		c.setI(settings::verbose >= printLevel::printNonCrit, check, I); // check is false, so we don't throw an error if the resulting battery state is valid
	}
	catch (int e)
	{
	...
	try
	{
		c.getVoltage(settings::verbose >= printLevel::printNonCrit, &v, &ocvp, &ocvn, &etap, &etan, &rdrop, &tem);
	...
	{
		if constexpr (settings::verbose >= printLevel::printNonCrit)
...
int BasicCycler::CC_t_V(double I, double dt, bool blockDegradation, double time, double Vupp, double Vlow, double *ahi, double *whi, double *timei)
{
	...
	{
		std::cerr << "Error in BasicCycler::CC_t_V. The total time " << time << " is not a multiple of the time step dt " << dt << '\n';
	...
	const int ttot = static_cast<int>(time / dt);					   // number of time steps needed in total
	const int nstore = static_cast<int>(CyclingDataTimeInterval / dt); // number of time steps between two data collection points
	...
	for (int t = 0; t < ttot; t++)
	{
		...
		if constexpr (settings::verbose >= printLevel::printCyclerDetail)
			std::cout << "BasicCycler::CC_t_V is applying a current of " << I << "A in iteration " << t
					  << " with so far " << tt << " seconds done, cell voltage " << v << '\n';
		...
		{
			// follow the current
			c.ETI(settings::verbose >= printLevel::printCrit, dt, blockDegradation);
		...
		{ // we are charging so stop if the voltage is above the upper limit
			c.setStates(s2, Iprev);
			endcriterion = 2;
			break;
		}
		else if (I > 0 && v < Vlow)
		{ // we are discharging so stop if the voltage is below the limit
			c.setStates(s2, Iprev);
			endcriterion = 3;
			break;
		}
		else if (v > c.getVmax())
		{ // the cell has exceeded the maximum voltage limit
			c.setStates(s2, Iprev);
			endcriterion = -2;
			break;
		}
		else if (v < c.getVmin())
		{ // the cell has exceeded the minimum voltage limit
			c.setStates(s2, Iprev);
			endcriterion = -3;
			break;
		}
		else
		{ // this is a valid iteration and we can to store the results
			...
			{
				if constexpr (settings::verbose >= printLevel::printCyclerDetail)
					std::cout << "BasicCycler::CC_t_V is storing cycling data in time step " << t << '\n';
...
void BasicCycler::CC_halfCell_full(double I, double dt, bool pos, std::vector<double> &OCVi, double *ahi, bool isWritten)
{
	...
	try
	{
		c.setI(settings::verbose >= printLevel::printCrit, check, I); // set the current
		c.getVoltage(settings::verbose >= printLevel::printCrit, &V, &OCVpi, &OCVni, &etapi, &etani, &Rdropi, &tempi);
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
	...
	while (limit)
	{
		...
		try
		{
			...
			c.ETI_electr(settings::verbose >= printLevel::printNonCrit, I, dt, blockDegradation, pos);						  // cycle only one electrode
			c.getVoltage(settings::verbose >= printLevel::printNonCrit, &V, &OCVpi, &OCVni, &etapi, &etani, &Rdropi, &tempi); // get the voltage
		...
		{																 // getVoltage throws an error if the min or max concentration has been reached
			if constexpr (settings::verbose >= printLevel::printNonCrit) // so this is a non critical error (we can expect it to happen because we want to (dis)charge an electrode to its min or max concentration
				std::cout << "Error in BasicCycler::CC_halfCell_full when OCVp = " << OCVpi << " and OCVn = " << OCVni << '\n';
...
void BasicCycler::findCVcurrent_recursive(double Imin, double Imax, int sign, double Vset, double dt, bool blockDegradation, double *Il, double *Vl)
{
	...
	bool vmax = Vset > c.getVmax(); // check if the maximum voltage is below the cell maximum voltage
	if (vmax)
		std::cerr << "Error in BasicCycler::findCVcurrent_recursive. The voltage " << Vset << " is too high. The maximum value is " << c.getVmax() << '\n'
				  << std::flush;
	bool vmin = Vset < c.getVmin(); // check if the minimum voltage is above the cell minimum voltage
	if (vmin)
		std::cerr << "Error in BasicCycler::findCVcurrent_recursive. The voltage " << Vset << " is too low. The minimum value is " << c.getVmin() << '\n'
				  << std::flush;
	if (vmax || vmin)
		throw 1005;
	...
	if (std::abs(dI) < 1e-6)
	{
		...
		c.setI(settings::verbose >= printLevel::printCrit, check, Imin);
		c.ETI(settings::verbose >= printLevel::printCrit, dt, blockDegradation);
		c.getVoltage(settings::verbose >= printLevel::printCrit, &Vtest, &ocvp, &ocvn, &etap, &etan, &rdrop, &tem);
		*Il = Imin;
		*Vl = Vtest;
	...
	for (int i = 0; i < nstep + 1; i++)
	{
		...
		try
		{
			c.setI(settings::verbose >= printLevel::printCrit, check, Itest);											// set the current
			c.ETI(settings::verbose >= printLevel::printCrit, dt, blockDegradation);									// take the time step
			c.getVoltage(settings::verbose >= printLevel::printCrit, &Vtest, &ocvp, &ocvn, &etap, &etan, &rdrop, &tem); // check the voltage
		}
		catch (int e)
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
		...
		if constexpr (settings::verbose >= printLevel::printfindCVcurrentDetail)
		...
		if (0 < err && err < 0.001)
		{ // the error in undershoot and it is below 1%
			*Il = Itest;
			*Vl = Vtest;
		...
		{							   // we are overshooting the voltage, so this is an upper bound of the current
...
void BasicCycler::findCVcurrent(double Vset, double dt, bool blockDegradation, double *Il, double *Vl)
{
	...
	if (c.getI() != 0)
	{									  // if there is a current in the cell, assume this current is a relatively good approximation
	...
	{ // if there is no current in the cell, assume 0 < |Il| < 1C
		Imin = 0;
	...
	try
	{
		c.setI(settings::verbose >= printLevel::printCrit, true, 0); // set the current to 0 and check if the resulting voltage is valid (which should always be the case)
		c.ETI(settings::verbose >= printLevel::printCrit, dt, blockDegradation);
		c.getVoltage(settings::verbose >= printLevel::printCrit, &Vtest, &ocvp, &ocvn, &etap, &etan, &rdrop, &tem);
	...
	if (0 < err && err < 0.001)
	{ // the error is an undershoot and it is below 0.1%
		Itest = 0;
	...
	{ // if not, call the recursive search algorithm
		try
		{
			// restore the original battery state
			c.setStates(s, Iini);
...
int BasicCycler::CV_t_I(double Vset, double dt, bool blockDegradation, double time, double Icut, double *ahi, double *whi, double *timei)
{
	...
	if (remainder(time, dt) > 0.01)
	{
		std::cerr << "Error in BasicCycler::CV_t_I. The total time " << time << " is not a multiple of the time step dt " << dt << '\n';
		throw 1003;
	}
	...
	int nstore = static_cast<int>(CyclingDataTimeInterval / dt); // number of time steps it takes to cover 'CyclingDataTimeInterval' seconds
	...
	while (!found)
	{
		...
		if (std::abs(Il) <= Icut)
		{				  // if Icut is negative, this never happens
		...
		else
		{
			...
			try
			{
				c.setI(settings::verbose >= printLevel::printCrit, check, Il);											// set the current
				c.ETI(settings::verbose >= printLevel::printCrit, dt, blockDegradation);								// apply the current for one time step
				c.getVoltage(settings::verbose >= printLevel::printCrit, &v, &ocvp, &ocvn, &etap, &etan, &rdrop, &tem); // get the cell voltage
			}
			catch (int e)
			{
				if constexpr (settings::verbose >= printLevel::printCrit)
			...
			{ // discharging
				timeDis += dt;
				AhDis += std::abs(Il) * dt / 3600.0;
				WhDis += std::abs(Il) * v * dt / 3600.0;
			}
			else if (Il < 0)
			{ // charging
				timeCha += dt;
				AhCha += std::abs(Il) * dt / 3600.0;
				WhCha += std::abs(Il) * v * dt / 3600.0;
			}
			else
			{ // resting
				timeRes += dt;
...
int BasicCycler::CC_t_CV_t(double I, double dt, bool blockDegradation, double time, double Vupp, double Vlow, double *ahi, double *whi, double *timei)
{
	...
	if (remainder(time, dt) > 0.01)
	{
		std::cerr << "Error in BasicCycler::CC_t_CV_t. The total time " << time << " is not a multiple of the time step dt " << dt << ".\n";
		throw 1003;
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCrit) // this was considered a critical error in CC_t_V so print that in this case, it wasn't a critical error and we might recover from it
		...
		try
		{
			...
			c.getVoltage(settings::verbose >= printLevel::printCrit, &v, &ocvp, &ocvn, &etap, &etan, &rdrop, &T);
		}
		catch (int e2)
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
	...
	if (end1 == 1)
	{										 // no voltage limit was hit, so we should have completed the full cycle
	...
	{							   // we haven't completed the full time, so we need to do a CV for the remaining time
		...
		{
			if constexpr (settings::verbose >= printLevel::printCrit) // we don't know which voltage limit was hit
...
void BasicCycler::CC_V_CV_I(double Crate, double Vset, double Ccut, double dt, bool blockDegradation, double *ahi, double *whi, double *timei)
{
	...
	try
	{
		c.setI(settings::verbose >= printLevel::printCrit, check, 0);											// set the cell current to 0
		c.getVoltage(settings::verbose >= printLevel::printCrit, &v, &ocvp, &ocvn, &etap, &etan, &rdrop, &tem); // get the OCV
		if (v < Vset)
	...
	if (Ccut < Crate)
	{ // cutoff current is larger than the CC current, so we don't have to do a CV phase
...
int BasicCycler::followI(int nI, const std::vector<double> &I, const std::vector<double> &T, bool blockDegradation, int limit, double Vupp, double Vlow, double *ahi, double *whi, double *timei)
{
	...
	for (size_t i = 0; i < I.size(); i++)
	{
		...
		if (vlim == 2 || vlim == -2)
		{ // the upper voltage limit (or the maximum cell voltage) was reached
			vmaxlim = true;
			if constexpr (settings::verbose >= printLevel::printCyclerDetail)
				std::cout << "BasicCycler::followI has hit the upper voltage limit during step " << i << ".\n";
		}
		else if (vlim == 3 || vlim == -3)
		{ // the lower voltage limit (or the minimum cell voltage) was reached
			vminlim = true;
			if constexpr (settings::verbose >= printLevel::printCyclerDetail)
