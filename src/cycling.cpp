...
void CCCV(const struct slide::Model &M, std::string pref, const struct DEG_ID &degid, int cellType, const int verbose)
{
	/*
	...
	const double Vmax = c1.getVmax();		// voltage to which the cell should be charged, use the maximum cell voltage [V]
	const double Vmin = c1.getVmin();		// voltage to which to discharge the cell, use the minimum cell voltage [V]
...
void FollowCurrent(const struct slide::Model &M, std::string pref, const struct DEG_ID &degid, int cellType, int verbose)
{
	/*
